...
import { fetchBlogBySlug } from '@/app/_componenents/Helper';
      ...
      <div className="relative">
        ...
        <div className="absolute top-0 left-0 right-0 bottom-0 bg-gradient-to-t from-gray-900 via-transparent to-transparent text-white p-6 flex flex-col justify-end">
          <div className="max-w-4xl mx-auto">
            <p className="text-sm mb-2">{new Date(blog.publishedAt).toLocaleDateString()}</p>
            <h1 className="text-4xl font-bold mb-2">{blog.title}</h1>
            <p className="text-sm">Newest Blog • 4 Min</p>
          </div>
        </div>
      </div>
      <div className="max-w-4xl mx-auto p-6 bg-white flex">
        ...
        <div className="w-3/4">
          ...
          <div className="prose lg:prose-xl">
            <p className="text-lg leading-relaxed mb-6">{renderContent(blog.description)}</p>
            <div className="text-lg leading-relaxed">{renderContent(blog.content)}</div>
          </div>
